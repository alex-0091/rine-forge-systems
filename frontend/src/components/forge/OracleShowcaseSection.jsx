...
export function OracleShowcaseSection({ onNavigate, onOpenAuditModal }) {
  ...
  const runSpeedSim = () => {
    ...
    setSpeedLogs([]);
    setTimeout(() => setSpeedLogs(p => [...p, '⚡ 00:01s — Inbound lead webhook received from Realtor.com portal']), 500);
    setTimeout(() => setSpeedLogs(p => [...p, '🧠 00:08s — AI Lead Classifier parsed intent: Pre-approved buyer ($1.4M budget)']), 1200);
    setTimeout(() => setSpeedLogs(p => [...p, '📱 00:19s — Autonomous 2-way SMS sent with interactive 3D virtual tour link']), 2000);
    setTimeout(() => {
  ...
  return (
    <section id="case-studies" className="py-20 sm:py-28 border-b border-white/[0.08] bg-[#060a12] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        
        {/* Section Header */}
        <div className="text-center space-y-3 max-w-3xl mx-auto">
          <div className="text-xs font-mono font-bold text-indigo-400 tracking-widest uppercase">
        ...
        {/* Showcase Sub-tabs */}
        <div className="flex flex-wrap gap-2 p-1.5 bg-[#0c101a] border border-white/[0.08] rounded-2xl max-w-2xl mx-auto">
          <button
            onClick={() => {
              forgeAudioSynth.playClick();
              setActiveSubTab('oracle');
            }}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${
              activeSubTab === 'oracle' ? 'bg-indigo-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'text-slate-400 hover:text-white'
            }`}
          >
            Oracle AI (Microstructure)
          </button>
          <button
            onClick={() => {
              forgeAudioSynth.playClick();
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${
              activeSubTab === 'speed' ? 'bg-indigo-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            onClick={() => {
              forgeAudioSynth.playClick();
              setActiveSubTab('omnisync');
            }}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${
              activeSubTab === 'omnisync' ? 'bg-indigo-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)]' : 'text-slate-400 hover:text-white'
            }`}
          >
            OmniSync (Audio Triage)
          </button>
        </div>

        {/* --- TAB 1: ORACLE AI --- */}
        {activeSubTab === 'oracle' && (
          <div className="bg-[#0c101a] border border-white/[0.1] rounded-3xl p-6 sm:p-10 space-y-8 shadow-2xl animate-fadeIn">
            {/* Header & Badges */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-6">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-2xl font-black text-white">Oracle AI</h3>
                  <span className="text-[10px] font-mono px-2.5 py-0.5 bg-amber-500/10 text-amber-300 border border-amber-500/25 rounded font-bold">
                    INTERNAL ENGINEERING SHOWCASE
                  </span>
                </div>
                <p className="text-xs text-slate-300 font-mono">
                  Autonomous real-time market intelligence and orderbook microstructure prediction engine.
                </p>
              </div>
              ...
              <button
                ...
                className="px-4 py-2 bg-[#080c14] hover:bg-white/[0.06] text-slate-200 border border-white/[0.1] rounded-xl text-xs font-mono font-bold flex items-center gap-2 shrink-0 self-start sm:self-auto cursor-pointer"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${oracleLoading ? 'animate-spin text-indigo-400' : ''}`} />
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 text-center text-xs font-mono">
                <div className="p-3 bg-[#080c14] rounded-xl border border-white/[0.06] space-y-1">
            ...
            {oracleData ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-5 bg-[#080c14] border border-white/[0.08] rounded-2xl space-y-3 font-mono">
                  ...
                  <div className="space-y-1 text-xs text-slate-300 pt-2 border-t border-white/[0.08]">
                ...
                <div className="p-5 bg-[#080c14] border border-white/[0.08] rounded-2xl space-y-3 font-mono">
                  ...
                  <div className="space-y-1.5 text-xs text-slate-300">
                    <div className="p-2 bg-[#0c101a] rounded border border-white/[0.06] text-[11px]">{oracleData.microstructure?.bid_liquidity_depth}</div>
                    <div className="p-2 bg-[#0c101a] rounded border border-white/[0.06] text-[11px]">{oracleData.microstructure?.ask_liquidity_depth}</div>
                    <div className="p-2 bg-[#0c101a] rounded border border-white/[0.06] text-[11px] text-teal-400">Funding: {oracleData.microstructure?.funding_rate}</div>
                  </div>
                </div>
              </div>
            ) : (
            ...
            <div className="p-4 bg-[#080c14] rounded-2xl border border-white/[0.06] text-xs text-slate-400 leading-relaxed font-mono flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <strong className="text-slate-300">Engineering Note:</strong> Oracle AI is an internal technology showcase demonstrating our team's capabilities in low-latency event processing, asynchronous stream ingestion, and real-time probabilistic scoring.
              </div>
              <button
                onClick={() => {
                  forgeAudioSynth.playClick();
                  if (onOpenAuditModal) onOpenAuditModal({ whatToAutomate: 'High-Throughput Asynchronous Pipeline Engineering' });
                }}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl shrink-0 cursor-pointer"
              >
        ...
        {/* --- TAB 2: SPEED TO LEAD --- */}
        {activeSubTab === 'speed' && (
          <div className="bg-[#0c101a] border border-white/[0.1] rounded-3xl p-6 sm:p-10 space-y-6 shadow-2xl animate-fadeIn">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-4">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-2xl font-black text-white">Speed-to-Lead Qualification Pipeline</h3>
                  <span className="text-[10px] font-mono px-2.5 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/25 rounded font-bold">
                    PRODUCTION PIPELINE
                  </span>
                </div>
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-black rounded-xl text-xs flex items-center gap-2 shrink-0 cursor-pointer shadow-md"
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-mono text-xs">
              <div className="p-5 bg-[#080c14] rounded-2xl border border-white/[0.08] space-y-3">
                <div className="text-slate-400 uppercase font-bold text-[10px]">Simulated Inbound Webhook Payload:</div>
                <div className="p-3 bg-[#0c101a] rounded-xl border border-white/[0.06] text-slate-300 space-y-1">
              ...
              <div className="p-5 bg-[#080c14] rounded-2xl border border-white/[0.08] space-y-3">
                ...
                <div className="h-40 bg-[#0c101a] rounded-xl p-3 border border-white/[0.06] overflow-y-auto space-y-2">
                  {speedLogs.length === 0 ? (
        ...
        {activeSubTab === 'omnisync' && (
          <div className="bg-[#0c101a] border border-white/[0.1] rounded-3xl p-6 sm:p-10 space-y-6 shadow-2xl animate-fadeIn">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-mono px-2.5 py-0.5 bg-rose-500/10 text-rose-300 border border-rose-500/25 rounded font-bold">
                    PRODUCTION DISPATCH
                  </span>
                </div>
            ...
            <div className="p-5 bg-[#080c14] rounded-2xl border border-white/[0.08] space-y-3 font-mono text-xs">
              <div className="text-slate-400 uppercase font-bold text-[10px]">Triage Dispatch Ticket:</div>
              {omniTicket ? (
                <div className="space-y-2">
                  <div className="p-3 bg-[#0c101a] rounded-xl border border-white/[0.06]">
                    <span className="text-slate-400 block text-[10px]">Caller:</span>
                    <span className="text-white font-bold">{omniTicket.caller}</span>
                  </div>
                  <div className="p-3 bg-[#0c101a] rounded-xl border border-white/[0.06] font-sans text-slate-300">
                    <span className="text-slate-400 font-mono block text-[10px]">Whisper Transcript:</span>
                    "{omniTicket.audio}"
                  </div>
                  <div className="p-3 bg-[#0c101a] rounded-xl border border-white/[0.06] flex justify-between items-center">
